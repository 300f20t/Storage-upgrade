...
public class DisplayTileEntity extends RandomizableContainerBlockEntity implements GeoBlockEntity, WorldlyContainer {
	private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
	private NonNullList<ItemStack> stacks = NonNullList.<ItemStack>withSize(9, ItemStack.EMPTY);
	private final LazyOptional<? extends IItemHandler>[] handlers = SidedInvWrapper.create(this, Direction.values());

	public DisplayTileEntity(BlockPos pos, BlockState state) {
		super(StorageUpgradeModBlockEntities.DISPLAY.get(), pos, state);
	}
	...
	public void registerControllers(AnimatableManager.ControllerRegistrar data) {
		data.add(new AnimationController<DisplayTileEntity>(this, "controller", 0, this::predicate));
		data.add(new AnimationController<DisplayTileEntity>(this, "procedurecontroller", 0, this::procedurePredicate));
	}
	...
	public AbstractContainerMenu createMenu(int id, Inventory inventory) {
		return new DisplayGUIMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(this.worldPosition));
	}
